...
in the same PostgreSQL transaction as the immutable postings. Database
...
The HTTP API exposes public UUIDs; sequential bigint IDs remain internal.
...
concurrent transfer cannot violate lifecycle rules.
...
| `GET /metrics` | Prometheus-compatible application and financial metrics. |
...
| `GET` | `/readyz` | Public | Dependency readiness probe. |
...
- Private metrics, protected administrative surfaces, alerting, tracing, and SLOs.
